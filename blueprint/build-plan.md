...
- [x] 3. Video upload API — endpoint that accepts a video file upload, stores it to local disk, and creates a video record with status PROCESSING
- [x] 4. Upload UI — a simple frontend form to upload a video file, and a list page showing uploaded videos with their status
- [x] 5. FFmpeg transcoding (single quality) — run FFmpeg synchronously on the uploaded file to produce HLS output (playlist + segments), target: source resolution, capped bitrate (~2-3 Mbps H.264/AAC), ~6s HLS segments; update the video record to READY on success or FAILED on error
