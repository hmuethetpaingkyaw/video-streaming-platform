...
No unit test runner is configured yet in either package (the backend `test`
script is a placeholder, and the frontend has none). There is no required test
gate until one exists. Run `/tests` or `$tests` to add one and update this
section with the real test commands.
